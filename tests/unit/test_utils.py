...
class TestUtils:
    def test__find_year_for_season_returns_correct_year(self):
        ...
        for month in season_start_matrix:
            ...
            flexmock(utils).should_receive("_date_today").and_return(mock_datetime)
...
<td class="right " data-stat="runs_per_game">4.10</td>"""
    ...
    def test_pulling_local_file(self, *args, **kwargs):
        ...
        # fpath = os.path.join(
        #     os.path.abspath(os.path.join(os.path.abspath(os.path.join(os.getcwd(), os.pardir)), os.pardir)),
        #     'VERSION')
        fpath = "VERSION"
        output = utils._pull_page(local_file=fpath)
