...
from pysports.api.constants import AWAY, HOME
from pysports.api.decorators import float_property_decorator, int_property_decorator
from pysports.api.nba.constants import (
    BOXSCORE_ELEMENT_INDEX,
    BOXSCORE_SCHEME,
    BOXSCORE_URL,
    BOXSCORES_URL,
)
from pysports.api.nba.player import AbstractPlayer, _int_property_decorator
...
class Boxscore:
    ...
    def _retrieve_html_page(self, uri):
        ...
        url_valid = utils.check_url_validity(url)
        if not url_valid:
            return None
        try:
            url_data = pq(url=url)
